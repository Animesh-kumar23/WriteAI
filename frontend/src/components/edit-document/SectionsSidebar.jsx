    ...
    >
      ...
      <div className="bg-white px-2 py-3 opacity-0 flex items-center gap-1 absolute right-0 transition-opacity duration-200 group-hover:opacity-100 group-focus-within:opacity-100 shadow-lg rounded-l">
        <Button
          ...
          onClick={(e) => {
            e.stopPropagation();
            ongenerateDocumentContent(index);
          }}
...
function SectionsSidebar({
  ...
  const chapterIds =
    document?.chapters?.map((chapter, index) => chapter?._id || `new-${index}`) ||
    [];
      ...
      <header className="border-b border-slate-200 p-4 bg-slate-50">
        ...
        <div className="mt-4">
          ...
          <p className="text-xs text-slate-500 mt-1">
            ...
            {document.chapters.length === 1 ? "chapter" : "chapters"}
          </p>
        </div>
      </header>
              ...
              <ul className="space-y-2 p-4">
                {document.chapters.map((chapter, index) => (
                  <SortableItem
                    ...
                    ongenerateDocumentContent={ongenerateDocumentContent}
                  />
                ))}
              </ul>
            ) : (
