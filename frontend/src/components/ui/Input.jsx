  ...
  return (
    <div className="w-full grid grid-cols-1 gap-y-2">
      ...
      <div className="relative">
        {InputIcon && (
          <div className="pl-3 flex justify-center items-center pointer-events-none absolute inset-y-0 left-0">
            <InputIcon className="size-4 text-gray-400 dark:text-slate-500" />
          </div>
        )}
        ...
        <input
          ...
          id={name}
          name={name}
          className={`w-full h-11 bg-white dark:bg-slate-800 text-gray-900 dark:text-slate-50 text-sm placeholder-gray-400 dark:placeholder-slate-500 px-3 py-2 border rounded-xl transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-violet-500 focus:ring-offset-2 dark:focus:ring-offset-slate-900 disabled:opacity-50 disabled:cursor-not-allowed ${
            InputIcon ? "pl-10" : ""
      ...
      {!error && helperText && (
        <p
          id={`${name}-helper`}
          className="text-gray-500 dark:text-slate-400 text-xs mt-1"
        >
          {helperText}
        </p>
      )}
