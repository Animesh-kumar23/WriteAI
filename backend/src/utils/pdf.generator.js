...
function _renderPdfContent(doc, document) {
  // COVER PAGE
  if (document.coverImage && !document.coverImage.includes("pravatar")) {
    const rel = document.coverImage.replace(/^\//, "");
    const imagePath = path.join(__dirname, "../../", rel);

    try {
      if (fs.existsSync(imagePath)) {
        doc.image(imagePath, { fit: [400, 550], align: "center", valign: "center" });
        doc.addPage();
      } else {
  ...
  if (document.subtitle && document.subtitle.trim()) {
    doc
  ...
  processMdContentForPdf(doc, document.content || "");
}
...
async function generatePdfBuffer(document) {
  return new Promise((resolve, reject) => {
    ...
    try {
      _renderPdfContent(doc, document);
      doc.end();
    } catch (error) {
      reject(error);
    }
...
async function generatePdf(document, res) {
